...
/**
 * {@link Validator} responsible for validating if the user {@link String} input
 * can be evaluated as
...
public class WarpCoordValidator extends Validator {
    ...
    /**
     * Takes the given {@link String} input and return if the given {@link String}
     * input can be
     * evaluated as conforming to either a DIGIT,DIGIT or DIGIT,DIGIT.DIGIT pattern.
     * Examples of valid
     * inputs:
     * <ul>
     * <li>1, 1</li>
     ...
     * @return if the given {@link String} input can be evaluated as conforming to
     *         either a
     *         DIGIT,DIGIT or DIGIT,DIGIT.DIGIT pattern.
     */
    @Override
    public boolean isValid(final String input) {
        var trimmedInput = input.trim(); // trim whitespace
        if (trimmedInput.length() <= 2
                || trimmedInput.length() > 5) { // can't be a valid NUMBER,NUMBER or NUMBER,NUMBER.NUMBER pattern!
            return false;
        }
