...
/**
 * The main Game class responsible for holding all relevant classes for the game
 * coordinating player
 * and enemy actions.
 */
public class Game implements GameModel {

    private String report = ""; // report for last action taken
    private Galaxy galaxy; // the game's galaxy
    private final int startingQuadrantX = 4; // starting quadrant X coordinate
    private final int startingQuadrantY = 4; // starting quadrant Y coordinate
    private Quadrant currentQuadrant; // current quadrant the player is in
    private Enterprise enterprise; // the player's ship

    /**
     * Construct an instance of {@link Game} with a generated list of 64
     * {@link Quadrant}s and a
    ...
    /**
     * Returns if the {@link Enterprise} can spare the amount of energy requested,
     * (if its energy is
     * at 1, it can't spare any energy as a reminder).
     *
     * @param energy - the amount of energy we want to check if the
     *               {@link Enterprise} can spare.
    ...
    /**
     * Returns the {@link Quadrant}s {@link String} symbols around the current
     * {@link Quadrant} in the
     * {@link Galaxy}.
     *
     * <p>
     * The return hashmap will always include the following keys: top topLeft
     * topRight right
     * bottomRight bottom bottomLeft left
     * </p>
     *
     * @return a {@code HashMap<String,String>} with the surrounding
     *         {@link Quadrant}s symbols linked
    ...
    /**
     * If there is any energy to spare, will spend as much of the requested amount
     * of energy as it can
     * to bolster the {@link Enterprise}s shields.
     *
     * @param requestedEnergySpend - amount of energy requested to spend on the
     *                             enterprises shields
     */
    @Override
    ...
    /**
     * Returns the currently stored string report for the outcome of the last
     * action.
     *
     * @return the currently stored string report for the outcome of the last
     *         action.
     */
    ...
    /**
     * Return the games {@link Enterprise} instance, useful for accessing internal
     * state of the
    ...
    public void scanQuadrant() {
        ...
        this.report = "Scanned Quadrant(" + this.currentQuadrant.getX() + "," + this.currentQuadrant.getY()
                + ") |";
    ...
    // JB: This is a bit overkill, but I want to expose them to generics somewhere,
    // and this is the
    // most sensible place I could find in the assessment given how small/focused it
    // is.
    // Realistically it could have just been a ArrayList<Entity> but then we lose
    // the teaching opportunity.

    /**
     * Returns a list of all symbols and their positions for the current
     * {@link Quadrant}.
     *
     * @param <T> - Generic type to wrap together the symbol method, the relevant
     *            position methods and
     *            the faction method.
    ...
    /**
     * Progresses the game state forward by one turn. Calls .tick() on the current
     * {@link Quadrant}
     * and calls .outOfFocusTick on {@link Quadrant}s that are not the current
     * quadrant.
     */
    public void turn() {
    ...
    /**
     * Handles the Phaser action, takes in how much energy the user has spent to
     * fire phasers. Phaser
     * energy is spread equally (rounding down) amongst the {@link Klingon}s in a
     * {@link Quadrant}.
     *
    ...
    public void phasers(final int energy, final Quadrant quadrant) {
        ...
        this.report = "Phaser (" + energy + ") at " + klingons + " targets, " + damagePerKlingon
                + " dmg each, "
                + klingonsDestroyed + " down";
    }

    /**
     * Takes a given course, we wish to fire a torpedo at. Handles a torpedo being
     * fired: travelling
     * across the current {@link Quadrant} sectors until it hits another
     * {@link Entity} or the edge of
     * the {@link Quadrant}.
     *
     * @param course   - course/direction we wish to fire the torpedo along in the
     *                 current
     *                 {@link Quadrant}.
    ...
    /**
     * Returns a string representation of which direction a given course will go
     * using utf 8 arrows.
     * 1:"→", 2:"↗", 3:"↑", 4:"↖", 5:"←", 6:"↙", 7:"↓", 8:"↘", otherwise: "X"
     *
     * @param course - given course value indicating direction
     * @return a string representation of which direction a given course will go
     *         using utf 8 arrows.
     */
    ...
    /**
     * Returns a {@link XyPair} int representation of the direction of movement
     * required to meet a
     * given course.
     *
     * @param course a double between 1 and 8.9
     * @return a {@link XyPair} representation of the direction of movement
     *         required.
     */
    ...
    /**
     * Move the given {@link Entity} within the {@link Quadrant}, by the course and
     * distance given or
     * until another {@link Entity} or edge of the {@link Quadrant} would be hit.
     *
     * @param course   the course/direction we wish the given {@link Entity} to move
     *                 within the given
     *                 {@link Quadrant}
    ...
    public void moveWithinQuadrant(final int course, final double distance) {
        ...
        this.report = "Moved" + this.getDirectionIndicatorArrow(course) + "(" + (int) Math.floor(distance)
    ...
    /**
     * Attempt to move the given {@link Entity} in the given {@link Quadrant},
     * adjusting the
     * {@link Entity} position by the given {@link XyPair} vector.
     *
     * <p>
     * If an edge or other {@link Entity} is encountered at the proposed new
     * location, the
     * report for the turn is updated and false is returned.
     * </p>
     *
     * <p>
     * If the proposed adjustment would be successful the {@link Entity} has its
     * position set to
     * the new position and true is returned.
     * </p>
     *
     * @param quadrant - {@link Quadrant} we wish to attempt to move the given
     *                 {@link Entity} in.
     * @param entity   - {@link Entity} we wish to attempt to move within the given
     *                 {@link Quadrant}
     * @param vector   - the adjustment we wish to make ot the {@link Entity}'s
     *                 position
    ...
    /**
     * Moves which {@link Quadrant} the {@link Game} is handling using the given
     * course, and given
     * number of quadrants to jump (rounded down).
     *
     * @param course   - the direction of the jump: 1:→, 2:↗, 3:↑, 4:↖, 5:←, 6:↙,
     *                 7:↓, 8:↘
     * @param distance - how many quadrants to jump through (we drop the decimal for
     *                 when jumping
     *                 between quadrants)
     */
    ...
    /**
     * Will attempt to move using a given vector one quadrant and returns a boolean
     * value indicating
     * if it succeeded or failed.
     *
     * @param vector - vector indicating how we wish to adjust the x and y of our
     *               position
    ...
    /**
     * Export out a stringified representation of the internal state of our Game,
     * specifically exports
     * the
     * {@link Enterprise} x,y,energy, shields, and torpedoes, as well as every
     * {@link sttrswing.model.Quadrant}s
     * x,y, number of stars, klingons and starbases. Example output structure: [e]
     * x:5 y:5 e:2500
     * s:500 t:10 | [q] x:0 y:0 s:111 | [q] x:0 y:1 s:002 | [q] x:0 y:2 s:110 | [q]
     * x:0 y:3 s:021 |
